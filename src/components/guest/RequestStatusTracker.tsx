      ...
      case 'PENDING':
        return (
          <span className="bg-[#FFF7E6] text-[#D97706] text-xs font-bold px-3 py-1 rounded-full border border-[#D97706]/30 flex items-center gap-1.5 shrink-0">
            <Clock className="w-3.5 h-3.5 text-[#D97706]" /> {t(currentLang, 'Pending Dispatch')}
          </span>
        );
      case 'ASSIGNED':
        return (
          <span className="bg-[#EFF6FF] text-[#2563EB] text-xs font-bold px-3 py-1 rounded-full border border-[#2563EB]/30 flex items-center gap-1.5 shrink-0">
            <User className="w-3.5 h-3.5 text-[#2563EB]" /> {t(currentLang, 'Staff Assigned')}
          </span>
        );
      case 'IN_PROGRESS':
        return (
          <span className="bg-[#EFF6FF] text-[#2563EB] text-xs font-bold px-3 py-1 rounded-full border border-[#2563EB]/30 animate-pulse flex items-center gap-1.5 shrink-0">
            <Truck className="w-3.5 h-3.5 text-[#2563EB]" /> {t(currentLang, 'Staff En Route')}
          </span>
        );
      case 'COMPLETED':
        return (
          <span className="bg-[#EAF8EF] text-[#16A34A] text-xs font-bold px-3 py-1 rounded-full border border-[#16A34A]/30 flex items-center gap-1.5 shrink-0">
            <CheckCircle2 className="w-3.5 h-3.5 text-[#16A34A]" /> {t(currentLang, 'Completed')}
          </span>
        );
      case 'CANCELLED':
        return (
          <span className="bg-[#F1F5F9] text-[#64748B] text-xs font-bold px-3 py-1 rounded-full border border-[#CBD5E1] flex items-center gap-1.5 shrink-0">
            <AlertCircle className="w-3.5 h-3.5 text-[#64748B]" /> {t(currentLang, 'Cancelled')}
          </span>
        );
      default:
    ...
    <div className="space-y-6 font-sans">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-bold text-[#171717] flex items-center gap-2">
            ...
            {t(currentLang, 'Live Concierge Status Tracker')}
          </h2>
          <p className="text-xs text-[#7C756B] font-medium">{t(currentLang, 'Real-time status of active requests for Room ')}{roomNumber}</p>
        </div>
        ...
        >
          <RefreshCw className="w-3.5 h-3.5 text-[#C6A15B]" /> {t(currentLang, 'Refresh')}
        </button>
      </div>
      ...
      {tickets.length === 0 && orders.length === 0 ? (
        <div className="bg-white border border-[#E2E8F0] rounded-2xl p-8 text-center text-[#7C756B] space-y-2 shadow-sm">
          ...
          <p className="font-semibold text-[#171717]">{t(currentLang, 'No Active Service Requests')}</p>
            ...
            >
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <h3 className="font-bold text-[#171717] text-base">{t(currentLang, tItem.title)}</h3>
                  <p className="text-xs text-[#7C756B] mt-0.5">{t(currentLang, tItem.description)}</p>
                </div>
              ...
              <div className="pt-3 border-t border-[#E2E8F0] flex items-center justify-between text-xs text-[#7C756B]">
                <span className="flex items-center gap-1.5 font-medium">
                  ...
                  {tItem.assignedStaff ? (
                    <span className="font-bold text-[#171717]">Assigned: {tItem.assignedStaff.name}</span>
                  ) : (
                    <span className="italic text-[#7C756B]">Awaiting staff assignment</span>
                  )}
                </span>
                <span className="text-[11px] text-[#7C756B] font-medium">
                  Requested {new Date(tItem.createdAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                </span>
              </div>
            </div>
          ))}
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs text-[#7C756B] mt-1 font-medium">
                    Items: {o.items?.map((i: any) => `${i.quantity}x ${t(currentLang, i.itemName)}`).join(', ')}
                  </p>
                </div>
                <span className="bg-[#FFF7E6] text-[#D97706] text-xs font-bold px-3 py-1 rounded-full border border-[#D97706]/30 flex items-center gap-1.5 shrink-0">
                  <ChefHat className="w-3.5 h-3.5 text-[#D97706]" /> {t(currentLang, 'Preparing in Kitchen')}
                </span>
              </div>
            </div>
          ))}
